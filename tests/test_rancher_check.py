...
def test_check(m: requests_mock.Mocker = None):
    ...
    assert len(components) == 165, "Number of Components does not match"
    assert len(relations) == 191, "Number of Relations does not match"
    assert len(health_check_states) == 158, "Number of Health does not match"
