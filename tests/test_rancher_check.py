...
def test_check(m: requests_mock.Mocker = None):
    ...
    assert len(components) == 156, "Number of Components does not match"
    assert len(relations) == 176, "Number of Relations does not match"
    assert len(health_check_states) == 156, "Number of Health does not match"
